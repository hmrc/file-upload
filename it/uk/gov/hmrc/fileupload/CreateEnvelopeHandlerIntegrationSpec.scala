...
/**
  ...
  *
  */
class CreateEnvelopeHandlerIntegrationSpec extends IntegrationSpec with EnvelopeActions {

  val formatter = DateTimeFormat.forPattern("yyyy-MM-dd'T'HH:mm:ss'Z'")
  val today = new DateTime().plusMinutes(10)

  feature("Create EnvelopeHandler") {

    scenario("Create a new EnvelopeHandler with empty body") {
      ...
      And("a new EnvelopeHandler record with no attributes will be created")

      And("the EnvelopeHandler ID will be returned in the location header")
      val locationHeader = response.header("Location").get
      locationHeader should fullyMatch regex ".*/file-upload/envelopes/[A-z0-9-]+$"
    }

    scenario("Create a new EnvelopeHandler using basic sample") {
      ...
      And("a new EnvelopeHandler record will be created")
  ...
  feature("Create EnvelopeHandler with id") {
