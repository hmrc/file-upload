...
class FileController(withBasicAuth: BasicAuth,
                     uploadBodyParser: (EnvelopeId, FileId, FileRefId, FileLength) => BodyParser[Future[JSONReadFile]],
  ...
  def upsertFile(id: EnvelopeId, fileId: FileId, fileRefId: FileRefId) = Action.async(uploadBodyParser(id, fileId, fileRefId, fileLength)) { request =>
    request.body.flatMap { jsonReadFile =>
      handleCommand(StoreFile(id, fileId, fileRefId, jsonReadFile.length)).map {
        ...
        case Xor.Left(EnvelopeMaxNumFilesExceededError) => ExceptionHandler(NOT_FOUND, s"Envelope Item Count exceeds maximum")
