...
object Service {
  ...
  type GetFileResult = GetFileError Xor FileFoundMongo
  case class FileFoundMongo(name: Option[String] = None, length: Long, data: Enumerator[Array[Byte]])
  ...
  def retrieveFile(getFileFromRepo: FileRefId => Future[Option[FileData]])
    ...
    (for {
      ...
      getFileFromRepo(file.fileRefId).map { maybeData =>
        val fileWithClientProvidedName = maybeData.map { d => FileFoundMongo(file.name, d.length, d.data) }
