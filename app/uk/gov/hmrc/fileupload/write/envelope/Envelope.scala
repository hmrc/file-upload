...
object Envelope extends Handler[EnvelopeCommand, Envelope] {
  ...
  val defaultMaxSizePerItem = 10 //MB

  type CanResult = Xor[EnvelopeCommandNotAccepted, Unit.type]

  override def handle = {
    case (command: CreateEnvelope, envelope: Envelope) =>
      envelope.canCreateWithFilesCapacityAndSize(command.maxSizePerItem.getOrElse(s"${defaultMaxSizePerItem}MB")).map(_ =>
  ...
  override def on = {
      case (envelope: Envelope, e: EnvelopeCreated) =>
        envelope.copy(state = Open, maxSizePerItem = e.maxSizePerItem.getOrElse(s"${defaultMaxSizePerItem}MB"))
...
case class Envelope(files: Map[FileId, File] = Map.empty, state: State = NotCreated, maxSizePerItem: String = "0MB") {

  def canCreateWithFilesCapacityAndSize(maxSizePerItem: String): CanResult = state.canCreateWithFilesCapacityAndSize(maxSizePerItem)
...
sealed trait State {
  ...
  def canCreateWithFilesCapacityAndSize(maxSizePerItem: String): CanResult = envelopeAlreadyCreatedError
...
object NotCreated extends State {
  ...
  override def canCreateWithFilesCapacityAndSize(maxSizePerItem: String): CanResult =
    if (!isValidFileSize(maxSizePerItem)) envelopeMaxSizePerItemError
    else successResult

  def isValidFileSize(size: String): Boolean = {
    val sizeRegex = "([1-9][0-9]{0,3})([KB,MB]{2})".r
    size.toUpperCase match {
      case sizeRegex(num, unit) =>
        unit match {
          case "KB" => true
          case "MB" => if (num.toInt <= Envelope.defaultMaxSizePerItem) true
          else false
        }
      case _ => false
    }
