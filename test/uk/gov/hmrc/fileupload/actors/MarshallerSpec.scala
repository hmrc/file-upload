	...
	"A Marshaller" should {
		...
		"unmarshall of an invalid json should fail" in {
			within(500 millis){
				...
				val future = marshaller ? UnMarshall(json, classOf[Envelope])
				Await.result(future, timeout.duration)
				future.value.get match {
					case Success(result) => result match{
						case _ : NoSuchElementException => () // expected
						case _ => fail("invalid json should not be marshalled")
					}
					case Failure(e) => e.getClass shouldBe classOf[NoSuchElementException]
				}
			}
		}
	}
