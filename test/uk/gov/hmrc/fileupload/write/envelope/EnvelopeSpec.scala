...
class EnvelopeSpec extends EventBasedGWTSpec[EnvelopeCommand, Envelope] {
  ...
  val envelopeCreated = EnvelopeCreated(envelopeId, Some("http://www.callback-url.com"), Some(new DateTime(0)),
    Some(Json.obj("foo" -> "bar")), Some(EnvelopeConstraints(10, 1000, 123, "application/pdf,image/jpeg,application/xml")))
  ...
  feature("CreateEnvelope") {
    ...
    scenario("Create new envelope") {
      ...
      givenWhenThen(
        --,
        CreateEnvelope(envelopeId, Some("http://www.callback-url.com"), Some(new DateTime(0)), Some(Json.obj("foo" -> "bar")), Some(EnvelopeConstraints(10, 1000, 123, "application/pdf,image/jpeg,application/xml")) ),
        envelopeCreated
      )
    }
    ...
    scenario("Create new envelope with number of items exceeding limit") {
      givenWhenThen(
        --,
        CreateEnvelope(envelopeId, Some("http://www.callback-url.com"), Some(new DateTime(0)), Some(Json.obj("foo" -> "bar")), Some(EnvelopeConstraints(122, 1000, 123, "application/pdf,image/jpeg,application/xml"))),
    ...
    scenario("Create new envelope with number of items < 1") {
      givenWhenThen(
        --,
        CreateEnvelope(envelopeId, Some("http://www.callback-url.com"), Some(new DateTime(0)), Some(Json.obj("foo" -> "bar")), Some(EnvelopeConstraints(0, 1000, 123, "application/pdf,image/jpeg,application/xml"))),
        InvalidMaxItemCountConstraintError
      )
    }
    ...
    scenario("Create new envelope with out of bounds max size per item constraint") {
      givenWhenThen(
        --,
        CreateEnvelope(envelopeId, Some("http://www.callback-url.com"), Some(new DateTime(0)), Some(Json.obj("foo" -> "bar")), Some(EnvelopeConstraints(12, 1000, Integer.MAX_VALUE, "application/pdf,image/jpeg,application/xml"))),
    ...
    scenario("Create new envelope with out of bounds max size constraint") {
      givenWhenThen(
        --,
        CreateEnvelope(envelopeId, Some("http://www.callback-url.com"), Some(new DateTime(0)), Some(Json.obj("foo" -> "bar")), Some(EnvelopeConstraints(12, Integer.MAX_VALUE, 23434, "application/pdf,image/jpeg,application/xml"))),
  ...
  feature("SealEnvelope") {
    ...
    scenario("Seal envelope with exceeding max item count") {
      ...
      eventsBuffer+=envelopeCreatedWithLimitedMaxItemConstraint(EnvelopeConstraints(2, 10000L, 123L, "application/pdf,image/jpeg,application/xml"))
      for(x <- 1 to 5){
    ...
    scenario("Seal envelope with exceeding max size") {

      val eventsBuffer = ListBuffer[EventData]()
      eventsBuffer+=envelopeCreatedWithLimitedMaxItemConstraint(EnvelopeConstraints(20, 10000L, 123L, "application/pdf,image/jpeg,application/xml"))
      for(x <- 1 to 5){
        eventsBuffer += FileQuarantined(envelopeId, FileId(s"fileId-$x"), FileRefId(s"fileRefId-$x"), 0, "test.pdf", "pdf", Some(5000L), Json.obj())
      }
      givenWhenThen(
        eventsBuffer.toList,
        SealEnvelope(envelopeId, "testRoutingRequestId", "DMS", "testApplication"),
        EnvelopeMaxSizeExceededError(10000L)
      )
    }
